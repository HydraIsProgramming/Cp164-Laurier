...
class BST:
    ...
    def _eq_aux(self, s_node, t_node):
            """
            ---------------------------------------------------------
            Determines whether two BSTs are equal recursively.
            Values in self and target are compared and if all values are equal
            and in the same location, returns True, otherwise returns False.
            Use: self._eq_recursive(s_node, t_node)
            ---------------
            Parameters:
                s_node - a bst (BST)
                t_node - a bst (BST)
            ...
            if s_node is None and t_node is None:
                ...
                equals = self._eq_aux(s_node._left, t_node._left) and self._eq_aux(
                    s_node._right, t_node._right)
    ...
    def is_balanced(self):
        ...
        if self._root is not None:
            ...
            root = self._root
            balanced = self._is_balanced_aux(root._left, root._right)
        return balanced
    
    def _is_balanced_aux(self, left, right):
            """
            ---------------------------------------------------------
            Returns whether a bst is balanced, i.e. the difference in
            height between all the bst's node's left and right subtrees is <= 1.
            ...
            if left is None and right is None:
                ...
                balanced = False
            elif left._height != right._height:
                balanced = False 
            else:
                balanced = self._is_balanced_aux(
                    left._left, left._right) and self._is_balanced_aux(right._left, right._right)        
            return balanced
    ...
    def is_valid(self):
        ...
        if self._count <= 1:
            ...
            valid = self._is_valid_aux(
                self._root, self._root._left, self._root._right)

        return valid
    
    def _is_valid_aux(self, root, left, right):
        """
        ---------------------------------------------------------
        Determines if a tree is a valid BST, i.e. the values in all left nodes
        are smaller than their parent, and the values in all right nodes are
        ...
        if left is None and right is None:
            ...
            valid = self._is_valid_aux(right, right._left, right._right)
            
        elif right is None:
            valid = self._is_valid_aux(left, left._left, left._right)
            
        elif left._value < root._value and right._value > root._value:
            
            if (max(left._height, right._height) + 1) == root._height:
                valid = True
    ...
    def levelorder(self):
        ...
        if self._root is not None:
            bst.insert(self._root)

            while bst.is_empty() is False:
                curr = bst.remove()
                values.append(deepcopy(curr._value))

                if curr._left is not None:
                    bst.insert(curr._left)

                if curr._right is not None:
                    bst.insert(curr._right)
